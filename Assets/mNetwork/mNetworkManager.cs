...
namespace mNetworkLibrary
{
	...
	public static class mNetworkManager
	{
		...
		internal static void UpdateAllBehaviours ()
		{
			// only update if connected
			if (mNetwork.networkState == mNetworkState.connected) {
				for (int i = 0; i < sceneNetworkIDs.Length; i++) {
					if (sceneNetworkIDs [i].targetObject != null) {
						sceneNetworkIDs [i].targetObject.SendMessage ("OnMNetworkUpdate", SendMessageOptions.DontRequireReceiver);
					}
				}
				for (int i = 0; i < gameNetworkIDs.Length; i++) {
					if (gameNetworkIDs [i].targetObject != null) {
						gameNetworkIDs [i].targetObject.SendMessage ("OnMNetworkUpdate", SendMessageOptions.DontRequireReceiver);
					}
				}
			}
		}
				...
				// check if it was recieved in the server socket
						...
						case mNetworkRPCMode.Others:
							...
							for (int a = 0; a < mNetwork.networkPlayers.Length; a++) {
								// check if the player is active
								// check if the player was not who sent it... since they don't need it back...
						...
						// if we're a dedicated server, we need this message too... since it won't be relayed to our client
							if (mNetwork.peerType == mNetworkPeerType.dedicatedServer) {
								local_ProcessRPC_ND (msg);
							}
